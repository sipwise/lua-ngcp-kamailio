...
TestNGCP = {} --class
    ...
    function TestNGCP:tearDown()
        KSR.pv.unset("$xavp(caller_dom_prefs)")
        KSR.pv.unset("$xavp(callee_dom_prefs)")
        KSR.pv.unset("$xavp(caller_prof_prefs)")
        KSR.pv.unset("$xavp(callee_prof_prefs)")
        KSR.pv.unset("$xavp(caller_peer_prefs)")
        KSR.pv.unset("$xavp(callee_peer_prefs)")
        KSR.pv.unset("$xavp(caller_usr_prefs)")
        KSR.pv.unset("$xavp(callee_usr_prefs)")
        KSR.pv.unset("$xavp(caller_real_prefs)")
        KSR.pv.unset("$xavp(callee_real_prefs)")
        KSR.pv.unset("$xavp(caller_fax_prefs)")
        KSR.pv.unset("$xavp(callee_fax_prefs)")
        self.ngcp = nil
    end
    ...
    function TestNGCP:test_caller_usr_load_empty_dom()
        ...
        self.cur:close()
        --
        self.con:execute("SELECT prefs.* FROM provisioning.voip_subscribers as usr LEFT JOIN prof_preferences AS prefs ON uKSR.profile_id = prefs.uuid WHERE uKSR.uuid = 'ae736f72-21d1-4ea6-a3ea-4d7f56b3887c'")  ;mc :returns(self.cur)
    ...
    function TestNGCP:test_callee_usr_load_prof_usr()
        ...
        self.cur:close()
        --
        self.con:execute("SELECT prefs.* FROM provisioning.voip_subscribers as usr LEFT JOIN prof_preferences AS prefs ON uKSR.profile_id = prefs.uuid WHERE uKSR.uuid = 'ah736f72-21d1-4ea6-a3ea-4d7f56b3887c'")  ;mc :returns(self.cur)
        self.cur:fetch(mc.ANYARGS)    ;mc :returns(pprof_vars:val("prof_2"))
        self.cur:fetch(mc.ANYARGS)    ;mc :returns(nil)
        self.cur:close()
        -- connection check
