...
TestNGCPProfilePrefs = {} --class
    ...
    function TestNGCPProfilePrefs:tearDown()
        ...
        KSR.log("info", "---TestNGCPProfilePrefs::cleaned---")
    end
    ...
    function TestNGCPProfilePrefs:test_caller_load()
        assertEvalToTrue(self.d.config)
        con:execute("SELECT prefs.* FROM provisioning.voip_subscribers as usr LEFT JOIN prof_preferences AS prefs ON uKSR.profile_id = prefs.uuid WHERE uKSR.uuid = 'ah736f72-21d1-4ea6-a3ea-4d7f56b3887c'")  ;mc :returns(self.cur)
