...
NGCPContractPrefs_MT.__tostring = function ()
    ...
    function NGCPContractPrefs:_get_location_id(contract, ip)
        ...
        local query = string.format("SELECT location_id FROM provisioning.voip_contract_locations cl JOIN provisioning.voip_contract_location_blocks cb ON cb.location_id = cl.id WHERE cl.contract_id = %d AND _ipv4_net_from <= UNHEX(HEX(INET_ATON('%s'))) AND _ipv4_net_to >= UNHEX(HEX(INET_ATON('%s'))) LIMIT 1", contract, ip, ip)
        if string.find(ip, ':') ~= nil then
            query = string.format("SELECT location_id FROM provisioning.voip_contract_locations cl JOIN provisioning.voip_contract_location_blocks cb ON cb.location_id = cl.id WHERE cl.contract_id = %d AND _ipv6_net_from <= UNHEX(HEX(INET_ATON('%s'))) AND _ipv5_net_to >= UNHEX(HEX(INET_ATON('%s'))) <= LIMIT 1", contract, ip, ip)
        end
