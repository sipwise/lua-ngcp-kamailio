...
NGCPRealPrefs_MT.__tostring = function ()
    ...
    function NGCPRealPrefs:_contract_load(level, keys)
        ...
        local contract_keys = {}
        local values = KSR.xavp.get(xavp.contract.name, 0, 0)
    ...
    function NGCPRealPrefs:_peer_load(level, keys)
        ...
        local peer_keys = {}
        local values = KSR.xavp.get(xavp.peer.name, 0, 0)
        for _,v in pairs(keys) do
            local value = values[v]
            if value then
    ...
    function NGCPRealPrefs:_usr_load(level, keys)
        ...
        local real_values = {}
        local dom_values = KSR.xavp.get(xavp.dom.name, 0, 0)
        local prof_values = KSR.xavp.get(xavp.prof.name, 0, 0)
        local usr_values = KSR.xavp.get(xavp.uKSR.name, 0, 0)
        for _,v in pairs(keys) do
