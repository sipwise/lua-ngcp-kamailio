...
NGCPRealPrefs_MT.__tostring = function ()
    ...
    function NGCPRealPrefs:_contract_load(level, keys)
        ...
        local contract_keys = {}
        local values = KSR.pvx.xavp_get(xavp.contract.name)
    ...
    function NGCPRealPrefs:_peer_load(level, keys)
        ...
        local peer_keys = {}
        local values = KSR.pvx.xavp_get(xavp.peer.name)
        for _,v in pairs(keys) do
            local value = values[v]
            if value then
    ...
    function NGCPRealPrefs:_usr_load(level, keys)
        ...
        local real_values = {}
        local dom_values = KSR.pvx.xavp_get(xavp.dom.name)
        local prof_values = KSR.pvx.xavp_get(xavp.prof.name)
        local usr_values = KSR.pvx.xavp_get(xavp.usr.name)
        for _,v in pairs(keys) do
