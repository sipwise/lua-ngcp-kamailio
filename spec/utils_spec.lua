...
describe("utils", function()
    ...
    describe("table", function()
        ...
        it("merge", function()
            assert.same(simple_list, {1,2,3})
            ut.table.merge(simple_list, {1})
            assert.same(simple_list, {1,2,3})
            ut.table.merge(simple_list, {5})
            assert.same(simple_list, {1,2,3,5})
            ut.table.merge(simple_list, {5,4})
            assert.same(simple_list, {1,2,3,5,4})
        end)

        it("merge should not error with nil", function()
            ut.table.merge(nil, nil)
            ut.table.merge(nil, {})
        end)

        it("merge empty", function()
            local tmp = {}
            ut.table.merge(tmp, {1,2,3,5,4})
            assert.same(tmp, {1,2,3,5,4})
        end)
